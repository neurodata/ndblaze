...
from restutil import getURL, getURLTimed, generateURLBlosc, putURLTimed, generateURLBlaze
...
class BenchmarkTest:
  ...
  def __init__(self, result):
    ...
    self.nd_host = result.ndserver_name
    ...
    for i in range(0, number_of_processes, 1):
      ...
      if all([a<b for a,b in zip(range_args[1::2], self.imagesize)]):
        if self.write_tests:
          data = blosc.pack_array(np.ones( [len(self.channels)]+map(sub, range_args[1::2], range_args[::2])[::-1], dtype=self.datatype) * random.randint(0,255))
          self.data_list.append(data)
          self.fetch_list.append(generateURLBlaze(self.host, self.token, self.channels, self.resolution, range_args))
        else:
          self.fetch_list.append(generateURLBlaze(self.host, self.token, self.channels, self.resolution, range_args))
  ...
  def getProjInfo(self):
    ...
    info = json.loads(getURL('http://{}/ca/{}/info'.format(self.nd_host, self.token)))
...
def main():
  ...
  parser.add_argument("--server", dest="server_name", action="store", type=str, default="localhost/ndblaze", help="NDBlaze Server Name")
  parser.add_argument("--ndstore", dest="ndserver_name", action="store", type=str, default="localhost/nd", help="NDStore Server Name")
